  ...
  content = {
    services.crowdsec = {
      ...
      settings = {
        api = {
          server.enable = false;
          client.credentials_path = "${config.sops.secrets.crowdsec-localhoax-secrets.path}";
        };
        prometheus = {
          enabled = true;
          level = "full";
    ...
    services.crowdsec-firewall-bouncer = {
      ...
      settings = {
        log_mode = "stdout";
        mode = "nftables";
        api_key = ''''${LOCALHOAX_FIREWALL_APIKEY}'';
    ...
    systemd = {
      services = {
        ...
        crowdsec-firewall-bouncer = {
          ...
          serviceConfig.EnvironmentFile = config.sops.secrets.crowdsec-localhoax-fwenv.path;
        };
      };
      tmpfiles.rules = [
    ...
    sops.secrets = {
      crowdsec-localhoax-secrets = {
        sopsFile = "${arcanum.secretPath}/crowdsec-localhoax.yaml";
        owner = "crowdsec";
        format = "yaml";
      };
      crowdsec-localhoax-fwenv = {
        sopsFile = "${arcanum.secretPath}/crowdsec-localhoax.yaml";
        owner = "crowdsec";
        format = "yaml";
      };
