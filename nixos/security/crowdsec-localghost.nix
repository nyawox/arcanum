  ...
  content = {
    services.crowdsec = {
      ...
      settings = {
        api = {
          server.enable = false;
          client.credentials_path = "${config.sops.secrets.crowdsec-localghost-secrets.path}";
        };
        prometheus = {
          enabled = true;
          level = "full";
    ...
    services.crowdsec-firewall-bouncer = {
      ...
      settings = {
        log_mode = "stdout";
        mode = "nftables";
        api_key = ''''${LOCALGHOST_FIREWALL_APIKEY}'';
    ...
    systemd = {
      services = {
        ...
        crowdsec-firewall-bouncer = {
          ...
          serviceConfig.EnvironmentFile = config.sops.secrets.crowdsec-localghost-fwenv.path;
        };
      };
      tmpfiles.rules = [
    ...
    sops.secrets = {
      crowdsec-localghost-secrets = {
        sopsFile = "${arcanum.secretPath}/crowdsec-localghost.yaml";
        owner = "crowdsec";
        format = "yaml";
      };
      crowdsec-localghost-fwenv = {
        sopsFile = "${arcanum.secretPath}/crowdsec-localghost.yaml";
        owner = "crowdsec";
        format = "yaml";
      };
