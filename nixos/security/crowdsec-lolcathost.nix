  ...
  content = {
    services.crowdsec = {
      ...
      settings = {
        api = {
          server.enable = false;
          client.credentials_path = "${config.sops.secrets.crowdsec-lolcathost-secrets.path}";
        };
        prometheus = {
          enabled = true;
          level = "full";
    ...
    sops.secrets.crowdsec-lolcathost-secrets = {
      sopsFile = "${arcanum.secretPath}/crowdsec-lolcathost.yaml";
      owner = "crowdsec";
      format = "yaml";
    };
  };
